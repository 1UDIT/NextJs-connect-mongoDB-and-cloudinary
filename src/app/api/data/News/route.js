...
import NewsModelTime from "@/app/models/NewsApi";
    ...
    if (basicAuth) {
        ...
        if (user === process.env.userLogin && pwd === process.env.passWord) {
            try {
                ...
                const cloudinary_id = result.public_id;
                try {
                    const newTask = new NewsModelTime({
                        title: body.title,
                        profile_img: result.secure_url,
                        cloudinary_id: result.public_id,
                        description: body.description,
                    });
                    let myPost = await db.collection("news").insertOne(newTask);
                    console.log(myPost, "myPOSTSS");
                    // return the posts
                    return NextResponse.json(
                        {
                            message: myPost
